...
export default class Search {
    ...
    sendRequest() {
      ...
      .then((serverResponseData) => {
        this.renderResultsHTML(serverResponseData.data)
      })
      .catch(() => {
    ...
    renderResultsHTML(posts) {
      if (posts.length) {
        ...
        <div class="list-group-item search-nav"><strong>Search Results</strong> (${posts.length > 1 ? `${posts.length} items found` : '1 item found'})</div>
          ...
          this.resultsArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm">Sorry, couldn't find any results.</p>`
      }
      this.hideLoaderIcon();
    ...
    openOverlay() {
      ...
      setTimeout(() => this.inputField.focus(), 30)
    }
